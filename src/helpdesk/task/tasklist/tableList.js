                ...
                filteredDisplayValues.map((display,index)=> {
                  if(display.type==='important' || display.type==='invoiced' ) {
                    ...
                    return <th key={display.value} className="row" colSpan={'1'} style={{color: '#0078D4', paddingLeft: "1px", paddingRight: "1px"}}>
                      <div
                        onClick={() => {
                          if( !tasks.some( (item) => item.checked ) ){
                ...
                if(display.type==='important' || display.type==='invoiced'){
                  ...
                  return <th key={display.value} colSpan={'1'} >
                    <Checkbox
                      ...
                      value = { tasks.every((item) => item.checked ) }
                      label = ""
          ...
          </tr>
          {
            ...
            .filter((item) =>
            {
              ...
              .every((display)=> {
                ...
                let result = value.toString().toLowerCase().includes(attributesFilter[display.value].toLowerCase());
                return result;
              });
            }).map((item)=>
            <tr
