  ...
  const renderCommandbar = () => {
    return ( <
      div className = "task-add-layout row" > {
        ...
        invoiced &&
          <
          div className = "inline-warning-message center-hor" > {
            t( 'taskInvoiced' )
          } < /div>
      } <
  ...
  const renderTitle = () => {
    return ( <
      ...
      h2 className = "center-hor" > {
        ...
        i className = {
          classnames( {
            "fa-pen": !taskTitleEdited,
            "fa-save": taskTitleEdited
          }, "fa clickable center-hor color-link" )
        }
        onClick = {
          () => setTaskTitleEdited( !taskTitleEdited )
        }
        />
      } {
        invoiced && inModal && < div className = "inline-warning-message center-hor" > {
            t( 'taskInvoiced' )
          } < /div> } <
          /div>
      );
        ...
        return ( <
          div className = "task-info ml-auto" > {
            ...
            ( <
              ...
              DatePicker className = "form-control hidden-input bolder p-0 text-right width-95"
              selected = {
                pendingDate
              }
              disabled = {
    ...
    const layoutComponents = {
      Project: ( <
        div > {
          ...
          div className = "disabled-info" > {
            project ? project.title : t( 'noProject' )
          } < /div>
        } {
          ( userRights.rights.taskProjectWrite && !invoiced ) &&
          <
          ...
          styles = {
            pickSelectStyle( [ 'noArrow', 'required', ] )
          }
          />
        } <
        /div>
      ...
      Assigned: ( <
        div > {
          ...
          options = {
            ( canAddUser ? [ translateSelectItem( {
              id: -1,
              title: '+ Add user',
              body: 'add',
      ...
      Status: ( <
        div > {
          ...
          styles = {
            pickSelectStyle( [ 'noArrow', 'colored', 'required', ] )
          }
          onChange = {
            changeStatus
          }
          options = {
            ( project ? toSelArr( project.project.statuses ) : [] ).filter( ( status ) => status.action !== 'Invoiced' )
          }
          />
        } <
        /div>
      ...
      Type: ( <
        div > {
          ...
          <
          div className = "disabled-info" > {
            taskType ? taskType.label : t( 'none' )
          } < /div>
        } {
          !projectAttributes.taskType.fixed && userRights.attributeRights.taskType.edit && !invoiced &&
            <
      ...
      Requester: ( <
        div > {
          ...
          <
          div className = "disabled-info" > {
            requester ? requester.label : t( 'none' )
          } < /div>
        } {
          !projectAttributes.requester.fixed && userRights.attributeRights.requester.edit && !invoiced &&
            <
      ...
      Company: ( <
        div > {
          ...
          options = {
            ( canAddCompany ? [ translateSelectItem( {
              id: -1,
              title: '+ Add company',
              body: 'add',
      ...
      StartsAt: ( <
        div > {
          ...
          <
          div className = "disabled-info" > {
            startsAt ? timestampToString( startsAt.valueOf() ) : t( 'noStartsAt' )
          } < /div>
        } {
          !projectAttributes.startsAt.fixed && userRights.attributeRights.startsAt.edit && !invoiced &&
            <
      ...
      Deadline: ( <
        div > {
          ...
          <
          div className = "disabled-info" > {
            deadline ? timestampToString( deadline.valueOf() ) : t( 'noDeadline' )
          } < /div>
        } {
          !projectAttributes.deadline.fixed && userRights.attributeRights.deadline.edit && !invoiced &&
            <
      ...
      Pausal: ( <
        div > {
          ...
          options = {
            translateAllSelectItems( booleanSelects, t )
          }
          />
        } <
        /div>
    ...
    const renderSelectsLayout1 = () => {
      return ( <
        ...
        Label className = "col-3 col-form-label" > {
          t( 'project' )
        } < span className = "warning-big" > * < /span></Label >
        <
        div className = "col-9" > {
        ...
        /div> {
          ...
          style = {
            ...
            div className = "p-r-10" >
            <
            Label className = "col-1-45 col-form-label" > {
              t( 'assignedTo' )
            } < span className = "warning-big" > * < /span></Label >
            <
        ...
        /div> {
          ...
          userRights.attributeRights.repeat.view &&
            ...
            div > {
              ...
              div className = "disabled-info" > {
                task.repeat ?
                `${t('repeatEvery')} ${task.repeat.repeatEvery} ${t(intervals.find( ( interval ) => interval.value === task.repeat.repeatInterval ).label).toLowerCase()}` : t( 'noRepeat' )
              } <
              /div> <
              /div> <
              /div>
    ...
    const renderMultiSelectTags = () => {
      return ( <
        Empty > {
          ...
          required = {
            userRights.attributeRights.tags.required
          }
          disabled = {
            projectAttributes.tags.fixed || invoiced
          }
          items = {
            toSelArr( project === null ? [] : project.project.tags )
          }
      ...
      return ( <
        ...
        /Label> {
          ...
          onClick = {
            ...
            <
            i className = {
              `fa fa-${!showDescription ? 'pen' : 'save' }`
            }
          /> {
          ...
          className = "btn-link btn-distance m-l-0 clickable" >
            <
    ...
    const renderCompanyPausalInfo = () => {
      ...
      return ( <
        ...
        span className = {
          classnames( {
            "warning-general": ( usedSubtaskPausal > taskWorkPausal )
          } )
        } > {
          ` ${usedSubtaskPausal}`
        } <
        ...
        span className = {
          classnames( {
            "warning-general": ( usedTripPausal > taskTripPausal )
          } )
        } > {
          ` ${usedTripPausal}`
        } <
    ...
    const renderSimpleSubtasks = () => {
      ...
      return ( <
        ShortSubtasks disabled = {
          !userRights.rights.taskSubtasksWrite || invoiced
        }
        items = {
          task.shortSubtasks
        }
    ...
    const renderVykazyTable = () => {
      ...
      return ( <
        Empty > {
          ...
          showColumns = {
            [ 'done', 'title', 'quantity', 'assigned', 'approved', 'actions' ]
          }
          showAdvancedColumns = {
            [ 'done', 'title', 'quantity', 'price', 'discount', 'priceAfterDiscount', 'actions' ]
          }
          autoApproved = {
            project ? project.project.autoApproved : false
          }
          ...
          taskAssigned = {
            assignedTo.filter( ( user ) => user.id !== null )
          }
...

        {
          ...
          showColumns = {
            [ 'done', 'title', 'quantity', 'price', 'total', 'approved', 'actions' ]
          }
          showTotals = {
            true
          }
    ...
    const renderModalUserAdd = () => {
      return ( <
        ...
        /Modal> {
          ...
          disabled = {
            !userRights.rights.projectWrite
          }
          projectID = {
            project.id
          }
