...
export default function TasksIndex (props) {
  ...
  const { data: tasksData, loading: tasksLoading } = useQuery(GET_TASKS, { variables: {filterId: filter.id, filter: filter.filter, projectId: ( generalFilter && generalFilter.project ? generalFilter.project.id : null ) }});
...
//  const { data: publicFiltersData, loading: publicFiltersLoading } = useQuery(GET_PUBLIC_FILTERS, { options: { fetchPolicy: 'network-only' }});
  ...
  const filterTasks = () => {
    ...
    if (tasksData){
      console.log("hello");
      console.log(localCache, localCache.milestone);
      filteredTasks = tasksData.tasks.filter(task => (localCache && localCache.milestone === null) || (task.milestone ? (task.milestone.id === localCache.milestone.id) : false ) )
    }
