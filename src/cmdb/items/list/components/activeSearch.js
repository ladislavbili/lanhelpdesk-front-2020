...
export default function ActiveSearch( props ) {
  ...
  if (
    (
      ...
      .filter( ( filterKey ) => (
        ( filterKey === 'active' && globalStringFilter[ filterKey ] !== null ) ||
        ( filterKey !== 'active' && globalStringFilter[ filterKey ].length !== 0 )
      ) )
      .length === 0
    )
  ) {
  ...
  if ( globalStringFilter !== null ) {
    usedFilter = [
      ...
      .map( ( filterKey ) => {
        if ( filterKey === 'active' ) {
          return globalStringFilter[ filterKey ] ? t( 'active2' ) : t( 'inactive2' );
        }
        return `${t(filterKey)}: ${globalStringFilter[ filterKey ]}`
      } )
    ]
  }
