...
class Filter extends Component {
  ...
  componentWillMount() {
    ...
    this.setState( {
      workTypes: toSelArr( this.props.taskTypes )
    } );
  }
  ...
  fetchData() {
    Promise.all(
      [
        database.collection( 'help-statuses' )
          .get(),
        database.collection( 'users' )
          .get(),
        database.collection( 'companies' )
          .get(),
        database.collection( 'help-task_types' )
          .get(),
      ] )
  ...
  deleteFilter() {
    if ( window.confirm( "Are you sure?" ) && this.props.filterID !== null ) {
      rebase.removeDoc( '/help-filters/' + this.props.filterID )
        .then( () => {
  ...
  applyFilter() {
    let body = {
      ...
      statusDateFrom: isNaN( new Date( this.state.statusDateFrom )
        .getTime() ) || this.state.statusDateFrom === '' ? '' : ( new Date( this.state.statusDateFrom )
        .getTime() ),
      statusDateTo: isNaN( new Date( this.state.statusDateTo )
        .getTime() ) || this.state.statusDateTo === '' ? '' : ( new Date( this.state.statusDateTo )
        .getTime() ),
      closeDateFrom: isNaN( new Date( this.state.closeDateFrom )
        .getTime() ) || this.state.closeDateFrom === '' ? '' : ( new Date( this.state.closeDateFrom )
        .getTime() ),
      closeDateTo: isNaN( new Date( this.state.closeDateTo )
        .getTime() ) || this.state.closeDateTo === '' ? '' : ( new Date( this.state.closeDateTo )
        .getTime() ),
      pendingDateFrom: isNaN( new Date( this.state.pendingDateFrom )
        .getTime() ) || this.state.pendingDateFrom === '' ? '' : ( new Date( this.state.pendingDateFrom )
        .getTime() ),
      pendingDateTo: isNaN( new Date( this.state.pendingDateTo )
        .getTime() ) || this.state.pendingDateTo === '' ? '' : ( new Date( this.state.pendingDateTo )
        .getTime() ),
      updatedAt: ( new Date() )
        .getTime()
    }
  ...
  renameFilter() {
    if ( this.props.filterData.title !== this.state.newFilterName &&
      ...
      rebase.updateDoc( '/help-filters/' + this.props.filterID, {
          title: this.state.newFilterName
        } )
        .then( () => {} );
    }
  }
